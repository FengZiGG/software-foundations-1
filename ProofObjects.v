...
Theorem nine_is_beautiful :
  beautiful 9.
Proof.
  apply b_sum with (n := 3) (m := 6). apply b_3.
...
Definition b_times2': forall n, beautiful n -> beautiful (2*n) :=
  fun n => fun H => (* b_sum n n H H *) admit.
...
Definition gorgeous_plus13_po: forall n, gorgeous n -> gorgeous (13+n):=
  fun n => fun H => g_plus3 _ (g_plus5 _ (g_plus5 n H)).
...
Definition conj_fact : forall P Q R, P /\ Q -> Q /\ R -> P /\ R :=
  fun P Q R => fun HPQ => fun HQR =>
  match HPQ with
    | conj _ _ HP _ =>
      match HQR with
        | conj _ _ _ HR => conj P R HP HR
      end
  end.
